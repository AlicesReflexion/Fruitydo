...
function insertdesc(dateText) {
  ...
  .done(function(data) {
    ...
    try {
      data.raw = sjcl.decrypt(decryptedpass, data.raw);
      data.fancy = markdown.toHTML(data.raw);
    } catch(e) {
 ...
 */
function saveEvent() {
  eventtext = $(this).parent().parent().find(".desc_input").val();
  eventtext = sjcl.encrypt(decryptedpass, eventtext);
  $(this).parent().parent().find(".desc_input").val(eventtext);
